...
public class JClient {
    ...
    public JClient (Player player, String ip, int port) {
        this.player = player;
        client = new Thread(new Connect(ip, port));
    }
    public static void main(String[] args) {
        Scanner scan = new Scanner(System.in);
        System.out.print("IP Addresses: ");
        JClient client = new JClient(new Player(scan.nextLine()), scan.nextLine(), 9797);
        client.start();
    }
    public void start() {
        client.start();
        ...
        public void run() {
            try {
                ...
                while(true) {
                    ...
                    if (line == null) return;
                    System.out.println("Server to " + player + ": " + line);
                    ...
                    else if (line.equals("DEALEND")) phase = CARD_CZAR;
